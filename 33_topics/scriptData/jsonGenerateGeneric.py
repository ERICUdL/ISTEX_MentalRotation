...
'''fileName : input file,
   key_words_list : list of items that can have to possible format : "key1" the keys to retrieve and keep for the new resulting json file, or "key1:keyBis1" "key1" is the key find in the data file, "keyBis1" is the key name to use in the resulting
   ...
   prec = characters that have to precede the selected json information in the resulting file containing all the documents, even with some keys missing, mostly "[" or ","
   prec_perf = characters that have to precede the selected json information in the resulting file containing the documents with all keys present in the given data file, mostly "[" or ","
...
def proceed(output_dir, input_dir, source, key_word_list, generate_key_words, verbose):
	print output_dir
...
if __name__ == "__main__":
	...
	parser.add_argument("--input_dir", default='Artificial_Intelligence', type=str) #directory name containing the original .json
	parser.add_argument("--output_dir", default="Artificial_IntelligenceJSONS", type=str)
	...
	parser.add_argument("--source", default='Istex', type=str) # specify from which library the documents were  extracted
	...
	print output_dir
	print input_dir
	proceed(output_dir, input_dir, source, key_word_list, generate_key_words, verbose)
